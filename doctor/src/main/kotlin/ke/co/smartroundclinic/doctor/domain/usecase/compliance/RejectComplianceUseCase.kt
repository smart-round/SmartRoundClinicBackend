...
class RejectComplianceUseCase(private val repository: ComplianceRepository) {
    suspend operator fun invoke(id: String, adminId: String, reason: String): DefaultResponse<ComplianceRes?> =
        repository.reject(id, adminId, reason).toDefaultResponse(
            failedStatusCode = HttpStatusCode.InternalServerError.value,
            successMessage = "Doctor application rejected",
