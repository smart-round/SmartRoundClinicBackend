...
class GetAdminPolicyGroupsUseCase(
    private val repository: PolicyGroupRepository,
    private val catalogRepository: PermissionCatalogRepository,
) {
    suspend operator fun invoke(adminUserId: String): DefaultResponse<List<PolicyGroupRes>?> =
        when (val result = repository.getAdminPolicyGroups(adminUserId)) {
            is Resource.Success -> {
                ...
                result.toDefaultResponse(HttpStatusCode.OK.value) {
                    groups.map { group -> group.toRes(group.permissions.mapNotNull { catalogsByKey[it] }) }
                }
            }
