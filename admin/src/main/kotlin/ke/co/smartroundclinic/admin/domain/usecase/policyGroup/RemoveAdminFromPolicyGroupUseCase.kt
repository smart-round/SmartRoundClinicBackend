...
class RemoveAdminFromPolicyGroupUseCase(private val repository: PolicyGroupRepository) {
    suspend operator fun invoke(policyGroupId: String, adminUserId: String): DefaultResponse<Nothing?> =
        when (val result = repository.removeAdmin(policyGroupId, adminUserId)) {
